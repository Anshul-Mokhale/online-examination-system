        ...
        <div class="main-panel">
            ...
            ";
                ...
                <div class="page-header">
                    ...
                    <h3 class="page-title">Results Of Practice Test</h3>
                ...
                echo "<div class = 'row'>";
                $id = $_SESSION['id'];
                $vss = "SELECT * FROM msub WHERE student_id = '$id'";
                $vresu = $mysql_connection->query($vss);
                if ($vresu->num_rows > 0) {
                    while ($row = $vresu->fetch_assoc()) {
                        $eid = $row['exam_id'];
                        $exmadata = "SELECT * FROM demoexam WHERE id = '$eid'";
                        ...
                        if ($examResu->num_rows > 0) {
                            while ($rew = $examResu->fetch_assoc()) {
                                ...
                                <div class='card'>
                                    <div class='card-body'>
                                        <h1 class='card-title'>Exam Name: " . $rew['examName'] . " " . $row['examNo'] . "</h1>
                                        <a href='viewModalResult.php?id=" . $_SESSION['id'] . "&examId=" . $rew['id'] . "&no=" . $row['examNo'] . "' class='btn btn-block btn-gradient-primary btn-lg font-weight-medium auth-form-btn'>View Result</a>
                                    </div>
                                </div>
                            </div>";
                            }
